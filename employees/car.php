...
<?php if ($article !== false) { ?>
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <select name="category_id" id="category" class="form-select">
                <?php foreach ($categories as $category) { ?>
                    <option value="1" <?php if (isset($car['category_id']) && $car['category_id'] == $category['id']) { ?>selected="selected" <?php }; ?>><?= $category['name'] ?></option>
                <?php } ?>
            </select>
        </div>
        ...
        <?php if (isset($_GET['id']) && isset($car['image'])) { ?>
            <p>
                <img src="<?= _ARTICLES_IMAGES_FOLDER_ . $car['image'] ?>" alt="<?= $car['title'] ?>" width="100">
